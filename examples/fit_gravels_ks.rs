...
fn main() {
    ...
    let model = ModelManager::new()
        ...
        .range(101) // Seed for rng for reproducibility.
        .runs(200) // Number of times to run the model per sampling point.
        .source_runs(1000) // Number of times to run the model per gravel source.
    ...
    // Change directory path for user, panics on invalid path
    fluvial.fit_rates_timed(&fg, "/home/erik/output/gravels_ks_200x_101.csv").unwrap();
}