...
fn main() {
    ...
    let fluvial = Fluvial::new()
        ...
        // .storage_rate_gravels(0.08335321)
        .capture_rate_gravels(0.118)
        .storage_rate_gravels(0.136)
        .turnover(&293.0)
        .manager(&model.clone()); // Load model parameters.

    let mut rec = fluvial.cherry_pick();
    utils::record(&mut rec, "/home/erik/output/stereotype_gravels_ch_1000.csv").unwrap();
}
